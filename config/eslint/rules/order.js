...
var object_groupby_1 = require("object.groupby");
var importType_1 = require("eslint-plugin-import/lib/core/importType");
var staticRequire_1 = require("eslint-plugin-import/lib/core/staticRequire");
var docsUrl_1 = require("eslint-plugin-import/lib/docsUrl");
var defaultGroups = ['builtin', 'external', 'parent', 'sibling', 'index'];
// REPORTING AND FIXING
function reverse(array) {
    return array
        .map(function (v) {
        return __assign(__assign({}, v), { rank: -v.rank });
    })
        .reverse();
}
...
function canReorderItems(firstNode, secondNode) {
    ...
    ].sort(), firstIndex = _a[0], secondIndex = _a[1];
...
function fixOutOfOrder(context, firstNode, secondNode, order) {
    ...
    var firstImport = makeImportDescription(firstNode) + " of `" + firstNode.displayName + "`";
    var secondImport = "`" + secondNode.displayName + "` " + makeImportDescription(secondNode);
    var message = secondImport + " should occur " + order + " " + firstImport;
    if (order === 'before') {
        context.report({
            ...
            fix: canFix &&
                (function (fixer) {
                    return fixer.replaceTextRange([firstRootStart, secondRootEnd], newCode +
...
var getNormalizedValue = function (node, toLowerCase) {
...
function getSorter(alphabetizeOptions) {
    ...
    return function importsSorter(nodeA, nodeB) {
        ...
        var result = 0;
        if (!(0, array_includes_1["default"])(importA, '/') && !(0, array_includes_1["default"])(importB, '/')) {
        ...
        if (!result && multiplierImportKind) {
            result =
                multiplierImportKind *
                    compareString(nodeA.node.importKind || DEAFULT_IMPORT_KIND, nodeB.node.importKind || DEAFULT_IMPORT_KIND);
        }
        return result;
    };
}
function mutateRanksToAlphabetize(imported, alphabetizeOptions) {
    var groupedByRanks = (0, object_groupby_1["default"])(imported, function (item) { return item.rank; });
...
function computePathRank(ranks, pathGroups, path, maxPosition) {
    for (var i = 0, l = pathGroups.length; i < l; i++) {
        var _a = pathGroups[i], pattern = _a.pattern, patternOptions = _a.patternOptions, group = _a.group, _b = _a.position, position = _b === void 0 ? 1 : _b;
        if (typeof pattern === 'function') {
            if (pattern(path)) {
                return ranks[group] + position / maxPosition;
            }
        }
        else if ((0, minimatch_1["default"])(path, pattern, patternOptions || { nocomment: true })) {
            return ranks[group] + position / maxPosition;
        }
    }
}
function computeRank(context, ranks, importEntry, excludedImportTypes) {
    ...
    if (importEntry.type === 'import:object') {
        ...
        impType = (0, importType_1["default"])(importEntry.value, context);
    }
...
function registerNode(context, importEntry, ranks, imported, excludedImportTypes) {
    ...
    if (rank !== -1) {
        imported.push(__assign(__assign({}, importEntry), { rank: rank }));
    }
}
...
function convertPathGroupsForRanks(pathGroups) {
    ...
    var transformed = pathGroups.map(function (pathGroup, index) {
        ...
        return __assign(__assign({}, pathGroup), { position: position });
    });
    var maxPosition = 1;
...
function makeNewlinesBetweenReport(context, imported, newlinesBetweenImports, distinctGroup) {
    ...
    var getIsStartOfDistinctGroup = function (currentImport, previousImport) {
    ...
    imported.slice(1).forEach(function (currentImport) {
        ...
        if (newlinesBetweenImports === 'always' ||
            ...
            if (currentImport.rank !== previousImport.rank &&
                ...
                newlinesBetweenImports !== 'always-and-inside-groups') {
                if ((distinctGroup && currentImport.rank === previousImport.rank) ||
...
module.exports = {
    ...
    create: function importOrderRule(context) {
        ...
        var pathGroupsExcludedImportTypes = new Set(options.pathGroupsExcludedImportTypes || [
            'builtin',
            'external',
            'object',
        ]);
        var alphabetize = getAlphabetizeConfig(options);
        var distinctGroup = options.distinctGroup == null
            ? defaultDistinctGroup
            : !!options.distinctGroup;
        var ranks;
        try {
            var _a = convertPathGroupsForRanks(options.pathGroups || []), pathGroups = _a.pathGroups, maxPosition = _a.maxPosition;
            var _b = convertGroupsToRanks(options.groups || defaultGroups), groups = _b.groups, omittedTypes = _b.omittedTypes;
            ranks = {
                groups: groups,
        ...
        return {
            ...
            CallExpression: function handleRequires(node) {
                if (!(0, staticRequire_1["default"])(node)) {
                    return;
                }
