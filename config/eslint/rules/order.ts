...
function fixOutOfOrder(context, firstNode, secondNode, order) {
  ...
  const firstImport = `${makeImportDescription(firstNode)} of \`${
    firstNode.displayName
  }\``;
  const secondImport = `\`${secondNode.displayName}\` ${makeImportDescription(
    secondNode,
  )}`;
...
function computePathRank(ranks, pathGroups, path, maxPosition) {
  for (let i = 0, l = pathGroups.length; i < l; i++) {
    ...
    if (typeof pattern === 'function') {
      ...
      minimatch(path, pattern, patternOptions || { nocomment: true })
    ) {
      return ranks[group] + position / maxPosition;
    }
  }
