...
import { DE_PROVINCE_FILTER_LIST, PAGE_SIZE } from './constants';
import grid from '../../../../../common/assets/styles/grid.legacy.css';
import sections from '../../assets/styles/sections.legacy.css';
import styles from './styles.legacy.css';
import { AmericanExpressRestaurantsProps } from './typings';

type ExtendedOrganization = Organization & {
  cloneContentUri?: string;
};

export const amexRestaurantsMsgs = defineMessages({
  provinceFilterPlaceholder: {
    id: 'app.amexRestaurants.filter',
    description:
      'Province filter placeholder text on amex restaurants overview page',
    defaultMessage: 'Alle',
  },
});

const doRenderFilterListItems = (
  ...
  return (
    <li key={`province-filter-item-${index}`}>
      <Link
        className={classNames(styles.FilterItem, {
          [styles.Active]:
            mapUrlToAmexProvinceEnum(province).label === label ||
            (mapUrlToAmexProvinceEnum(province).label === '' &&
              label ===
                intl.formatMessage(
                  amexRestaurantsMsgs.provinceFilterPlaceholder,
                )),
        })}
        path={url}
        label={label}
...
}: AmericanExpressRestaurantsProps) => {
  if (!mapUrlToAmexProvinceEnum(province.toLowerCase())?.amexProvinceEnum) {
    return <NotFound />;
  }
  ...
  return (
    <div className={classNames('restaurants-overview', styles.Wrapper)}>
      ...
      <div className={sections.Container}>
        <div className={grid.Row}>
          <div className={grid.ColXs24}>
            <ExpansionPanel title={mapUrlToAmexProvinceEnum(province).label}>
              <ul className={styles.FilterItemsList}>
                {DE_PROVINCE_FILTER_LIST.map(renderFilterListItems)}
              </ul>
            </ExpansionPanel>
          </div>
        </div>
      ...
      {(data?.environment?.globalSearch &&
        ...
        <div className={sections.Container}>
          <FormattedMessage
            id="app.amexrestaurants.nothingfound"
...
const extendWithHandler = withHandlers({
  ...
  renderFilterListItems: (props: any) => (item, index) =>
    doRenderFilterListItems(item, index, props.province, props.intl),
});
