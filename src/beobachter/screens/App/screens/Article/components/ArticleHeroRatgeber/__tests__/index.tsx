...
jest.mock('../../ArticleHero/components/ImageGalleryHero');
jest.mock('../../ArticleHero/components/ArticleImage');
jest.mock('../../ArticleHero/components/ArticleVideo');
jest.mock('../../ArticleHero/components/ArticleDefaultRatgeber');
...
describe('[Component] ArticleImage', () => {
  ...
  it('Should render the ArticleDefaultRatgeber component as a fallback', () => {
    // @ts-ignore
    initialProps.article.heroImageBody[0].__typename = '';
    const { queryByTestId } = render(
      <ReduxProvider initialState={initialState}>
        {/* @ts-ignore */}
        <Component {...initialProps} />
    ...
    expect(
      /* @ts-ignore TODO: TS2531 ->  Object is possibly 'null'. */
      queryByTestId('article-image-articledefaultratgeber-wrapper').innerHTML,
    ).not.toBe('');
  });
});
