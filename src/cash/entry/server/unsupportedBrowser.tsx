export default () => `<!DOCTYPE html>
<html lang="de-CH">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <title>Browser nicht unterstützt</title>
    <script type="text/javascript">
      var utag_data = {
        cms_page_type: 'ErrorPage',
      };
    </script>
    <style type="text/css">
      html,
      body {
        margin: 0;
        padding: 0;
      }

      body {
        -webkit-font-smoothing: antialiased;
      }

      .container {
        margin-left: auto;
        margin-right: auto;
        width: calc(100% - 38px);
      }

      .wrapper {
        margin: 40px 0 64px;
      }

      .headerWrapper {
        align-items: flex-start;
        display: flex;
      }

      .iconWrapper {
        height: 72px;
        width: 72px;
      }

      .icon {
        height: 72px;
        width: 72px;
        max-height: 72px;
        max-width: 72px;
      }

      .title {
        color: #292e32;
        font-family: "Source Serif Pro","Times New Roman",Times,serif;
        font-size: 25px;
        font-weight: bold;
        line-height: 30px;
        margin-left: 16px;
        min-height: 72px;
      }

      .description {
        color: #292e32;
        font-family: "Source Sans Pro",Arial,Helvetica,sans-serif;
        font-size: 20px;
        line-height: 28px;
        margin-top: 16px;
        text-align: left;
      }

      .linkWrapper {
        display: flex;
        flex-direction: column;
        margin-top: 28px;
      }

      .link {
        font-family: "Source Sans Pro",Arial,Helvetica,sans-serif;
        font-size: 16px;
        font-weight: 700;
        text-align: center;
        text-decoration: none;
        background-color: #ffffff;
        border-radius: 4px;
        color: #107B8E;
        margin-bottom: 8px;
        padding: 16px 24px;
        width: 80%;
        border: 1px solid #107B8E;
        display: flex;
        align-items: center;
        justify-content: space-between;
      }

      .link:hover {
        color: #004F61;
        border-color: #004F61;
      }

      .externalLinkIcon {
        height: 18px;
      }

      .externalLinkIcon > g > g {
        fill: #107B8E;
      }

      .link:hover .externalLinkIcon > g > g {
        fill: #004F61;
      }

      @media (min-width: 760px) {
        .container {
          width: 702px;
        }

        .wrapper {
          margin: 80px 0;
        }

        .title {
          font-size: 45px;
          line-height: 50px;
          margin-left: 32px;
        }

        .description {
          font-size: 22px;
          line-height: 32px;
          margin-left: 104px;
          margin-top: 8px;
        }


        .linkWrapper {
          flex-direction: row;
          align-items: center;
        }

        .link {
          width: 248px;
          margin-right: 24px;
        }
      }

      @media (min-width: 960px) {
        .container {
          width: 796px;
        }

        .link {
          font-size: 18px;
        }
      }

      @media (min-width: 1680px) {
        .container {
          width: 1272px;
        }
      }
    </style>
  </head>
  <body>
    <div class="wrapper">
      <div class="container">
        <div class="headerWrapper">
          <div class="iconWrapper">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 72 72">
              <g>
                <circle
                  fill="#EDECE8"
                  stroke="#EDECE8"
                  stroke-miterlimit="10"
                  cx="36"
                  cy="36"
                  r="35.5"
                />
                <path
                  fill="#FFF"
                  stroke="#3E3E3E"
                  stroke-width="2"
                  stroke-linejoin="round"
                  stroke-miterlimit="10"
                  d="M47.28 54.91H22.17a.76.76 0 01-.76-.76V23.42a1 1 0 01.3-.71l5.39-5.32c.19-.18.44-.29.7-.29h19.48c.42 0 .76.34.76.76v36.3c0 .41-.34.75-.76.75z"
                />
                <path
                  fill="none"
                  stroke="#3E3E3E"
                  stroke-width="2"
                  stroke-linejoin="round"
                  stroke-miterlimit="10"
                  d="M21.57 23.63h5.54c.55 0 1-.45 1-1v-5.54"
                />
                <path
                  fill="none"
                  stroke="#3E3E3E"
                  stroke-width="2"
                  stroke-miterlimit="10"
                  d="M35 30v9"
                />
                <g fill="#3E3E3E">
                  <circle cx="35" cy="42" r="1" />
                  <path
                    d="M35 41.5c.28 0 .5.22.5.5s-.22.5-.5.5-.5-.22-.5-.5.22-.5.5-.5m0-1c-.83 0-1.5.67-1.5 1.5s.67 1.5 1.5 1.5 1.5-.67 1.5-1.5-.67-1.5-1.5-1.5z"
                  />
                </g>
              </g>
            </svg>
          </div>

          <div class="title">Dieser Browser wird nicht mehr unterstützt</div>
        </div>

        <div class="description">
          Wir empfehlen dir, einen der folgenden Browser zu nutzen
        </div>

        <div class="linkWrapper">
          <a href="https://www.google.com/intl/de/chrome/" class="link">
            Google Chrome
            <svg
              class="externalLinkIcon"
              version="1.1"
              viewBox="0 0 11 11"
              height="22"
              width="22"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g fill="none" fill-rule="evenodd">
                <g
                  transform="translate(-587 -247)"
                  fill="#000"
                  fill-rule="nonzero"
                >
                  <g transform="translate(587 247)">
                    <path
                      id="a"
                      d="m5.9583 5.7444 3.3162-3.3543 0.90319 0.8284c0.15278 0.15391 0.32802 0.18107 0.52574 0.081481s0.29657-0.25802 0.29657-0.47531v-2.2951c0-0.15391-0.049428-0.28066-0.14828-0.38025-0.098856-0.099588-0.22018-0.14938-0.36397-0.14938h-2.5074c-0.23366 0-0.39318 0.10638-0.47855 0.31914-0.085376 0.21276-0.042688 0.39609 0.12806 0.55l0.95711 0.8284-3.3297 3.3407 0.70098 0.70617z"
                    />
                    <path
                      d="m9.2341 11c0.21569 0 0.39767-0.076955 0.54596-0.23086s0.22243-0.33951 0.22243-0.55679v-4.9568h-0.94363v4.7802h-8.0613v-8.1617h4.7451v-0.9642h-4.9608c-0.21569 0-0.39992 0.076955-0.5527 0.23086-0.15278 0.15391-0.22917 0.33951-0.22917 0.55679v8.5148c0 0.21728 0.076389 0.40288 0.22917 0.55679 0.15278 0.15391 0.33701 0.23086 0.5527 0.23086h8.4522z"
                    />
                  </g>
                </g>
              </g>
            </svg>
          </a>
          <a href="https://www.mozilla.org/de/firefox/new/" class="link">
            Mozilla Firefox
            <svg
              class="externalLinkIcon"
              version="1.1"
              viewBox="0 0 11 11"
              height="22"
              width="22"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g fill="none" fill-rule="evenodd">
                <g
                  transform="translate(-587 -247)"
                  fill="#000"
                  fill-rule="nonzero"
                >
                  <g transform="translate(587 247)">
                    <path
                      id="a"
                      d="m5.9583 5.7444 3.3162-3.3543 0.90319 0.8284c0.15278 0.15391 0.32802 0.18107 0.52574 0.081481s0.29657-0.25802 0.29657-0.47531v-2.2951c0-0.15391-0.049428-0.28066-0.14828-0.38025-0.098856-0.099588-0.22018-0.14938-0.36397-0.14938h-2.5074c-0.23366 0-0.39318 0.10638-0.47855 0.31914-0.085376 0.21276-0.042688 0.39609 0.12806 0.55l0.95711 0.8284-3.3297 3.3407 0.70098 0.70617z"
                    />
                    <path
                      d="m9.2341 11c0.21569 0 0.39767-0.076955 0.54596-0.23086s0.22243-0.33951 0.22243-0.55679v-4.9568h-0.94363v4.7802h-8.0613v-8.1617h4.7451v-0.9642h-4.9608c-0.21569 0-0.39992 0.076955-0.5527 0.23086-0.15278 0.15391-0.22917 0.33951-0.22917 0.55679v8.5148c0 0.21728 0.076389 0.40288 0.22917 0.55679 0.15278 0.15391 0.33701 0.23086 0.5527 0.23086h8.4522z"
                    />
                  </g>
                </g>
              </g>
            </svg>
          </a>
          <a href="https://www.microsoft.com/de-de/edge" class="link">
            Microsoft Edge
            <svg
              class="externalLinkIcon"
              version="1.1"
              viewBox="0 0 11 11"
              height="22"
              width="22"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g fill="none" fill-rule="evenodd">
                <g
                  transform="translate(-587 -247)"
                  fill="#000"
                  fill-rule="nonzero"
                >
                  <g transform="translate(587 247)">
                    <path
                      id="a"
                      d="m5.9583 5.7444 3.3162-3.3543 0.90319 0.8284c0.15278 0.15391 0.32802 0.18107 0.52574 0.081481s0.29657-0.25802 0.29657-0.47531v-2.2951c0-0.15391-0.049428-0.28066-0.14828-0.38025-0.098856-0.099588-0.22018-0.14938-0.36397-0.14938h-2.5074c-0.23366 0-0.39318 0.10638-0.47855 0.31914-0.085376 0.21276-0.042688 0.39609 0.12806 0.55l0.95711 0.8284-3.3297 3.3407 0.70098 0.70617z"
                    />
                    <path
                      d="m9.2341 11c0.21569 0 0.39767-0.076955 0.54596-0.23086s0.22243-0.33951 0.22243-0.55679v-4.9568h-0.94363v4.7802h-8.0613v-8.1617h4.7451v-0.9642h-4.9608c-0.21569 0-0.39992 0.076955-0.5527 0.23086-0.15278 0.15391-0.22917 0.33951-0.22917 0.55679v8.5148c0 0.21728 0.076389 0.40288 0.22917 0.55679 0.15278 0.15391 0.33701 0.23086 0.5527 0.23086h8.4522z"
                    />
                  </g>
                </g>
              </g>
            </svg>
          </a>
        </div>
      </div>
    </div>
  </body>
</html>`;
