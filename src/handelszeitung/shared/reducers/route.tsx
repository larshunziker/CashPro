...
export default (
  ...
  action: RouteStateActionHZ<Record<string, any>>,
): LocationState => {
  ...
  switch (action.type) {
    case ON_LOCATION_CHANGE: {
      ...
      if (__CLIENT__) {
        ...
        const adDivInReadVideo: HTMLElement | null =
          document.getElementById('teads-inread');
        ...
        if (adDivInReadVideo) {
          adDivInReadVideo.remove();
        }
        if (adDivSpotInReadVideo) {
          [].forEach.call(adDivSpotInReadVideo, (item: HTMLElement): void => {
            item.remove();
          });
        }
        // @TODO: this is a hotfix. We should contact RIAD that they'll use their own div elements
    ...
    case SET_SCREEN_READY: {
      ...
      if (global.Ads?.tracking) {
        /* @ts-ignore TODO: TS7017 ->  Element implicitly has an 'any' type because type 'typeof globalThis' has no index signature. */
        global.Ads.tracking.screenReady = Date.now();
      }
...
const doHandleTealium = (
  action: RouteStateActionHZ<Record<string, any>> & { hit_type?: string },
  forceTracking = false,
): void => {
  if (hasTealiumData(action.payload)) {
    tealiumTrackEvent({
      payload: {
        ...
        cms_user_subscription: action.payload.externalSubscription,
